...
(function () {
  ...
  function renderLiveTelemetry(d) {
    ...
    // Heatwave Notification & Push Alert Trigger
    if (d.heat_index_c > 38.0 || d.tinyml_heat_class === 'HEAT_EMERGENCY' || d.tinyml_heat_class === 'HEAT_WARNING') {
      if (!window._lastHeatNotifTime || (Date.now() - window._lastHeatNotifTime > 60000)) {
        window._lastHeatNotifTime = Date.now();
        const heatTitle = d.heat_index_c > 41.0 ? 'EXTREME HEATWAVE HAZARD' : 'HEATWAVE THERMAL STRESS';
        const heatBody = `Heat Index of ${d.heat_index_c ? d.heat_index_c.toFixed(1) : '39'}°C recorded. Shaded rest & cold ORS fluids required immediately.`;
        showDashboardToast(heatTitle, heatBody, d.heat_index_c > 41.0 ? 'danger' : 'warning');
        sendBrowserNotification(`⚠️ ${heatTitle}`, heatBody);
      }
    }
    ...
    // 3D Avatar Motion State Sync (Gemini AI + TinyML Multi-Sensor Analysis)
    if (window.AthenaAvatar) {
      if (d.fall_detected) {
        window.AthenaAvatar.setState('fallen');
      } else if (d.tinyml_class === 'RUNNING' || (d.is_moving && (d.accel_magnitude || 1) > 1.8)) {
        window.AthenaAvatar.setState('running');
      } else if (d.tinyml_class === 'WALKING' || d.is_moving) {
        window.AthenaAvatar.setState('walking');
      } else {
        window.AthenaAvatar.setState('breathing'); // Default to Breathing Idle 3D FBX model
      }
    }
  ...
  function renderAIAdvisory(adv) {
    ...
    // 3D Avatar Motion State Sync from Gemini AI Reasoning
    if (window.AthenaAvatar && adv.avatar_animation_state) {
      window.AthenaAvatar.setState(adv.avatar_animation_state);
    }
  ...
  // ==========================================================================
  // REAL-TIME NOTIFICATION SYSTEM (BROWSER PUSH & DASHBOARD TOAST)
  // ==========================================================================
  // Register Service Worker for Mobile Lock-Screen Push Notifications (YouTube Style)
  if ('serviceWorker' in navigator) {
    window.addEventListener('load', () => {
      navigator.serviceWorker.register('/sw.js').then(reg => {
        console.log('[PWA SW] Service Worker active for mobile lock-screen push alerts:', reg.scope);
      }).catch(err => {
        console.warn('[PWA SW] Service Worker registration warning:', err);
      });
    });
  }
    ...
    if ('Notification' in window) {
      Notification.requestPermission().then(permission => {
        ...
        if (permission === 'granted') {
          if (btn) btn.textContent = '🔔 Alerts Active';
          showDashboardToast('Push Notifications Active', 'You will receive real-time alerts for heatwaves & emergency events.', 'warning');
        } else {
          if (btn) btn.textContent = '🔕 Alerts Blocked';
        }
      });
    } else {
      alert('Browser Push Notifications not supported in this browser.');
    }
  }
  ...
  function showDashboardToast(title, message, level = 'warning') {
    const container = document.getElementById('toast-container');
    if (!container) return;

    const toast = document.createElement('div');
    toast.className = `toast-banner ${level === 'danger' ? 'danger' : 'warning'}`;
    
    toast.innerHTML = `
      <div class="toast-header">
        <span class="toast-title">${level === 'danger' ? '🚨' : '⚠️'} ${title}</span>
  ...
  function triggerEmergencyAlert({ title, desc, hr, spo2, heat }) {
    ...
    // Trigger Browser Push & Toast Banner Notification
    sendBrowserNotification(`🚨 ${title}`, desc);
    showDashboardToast(title, desc, 'danger');
  }
